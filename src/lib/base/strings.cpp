...
template<> string _cdecl _xf_str_(const string &f, const char *const value) {
	if (f != "s")
		throw Exception("format evil (string): " + f);
	return value;
}
...
template<> string _cdecl _xf_str_(const string& f, char* const value) { return _xf_str_<const char*>(f, value); }
//template<> string _cdecl _xf_str_(const string& f, const char* const value) { return _xf_str_<const char*>(f, value); }
