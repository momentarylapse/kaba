...
void SyntaxTree::AutoImplementAddVirtualTable(Command *self, Function *f, Class *t)
{
	if (t->vtable.num > 0){
		...
		Command *c = add_command_operator_by_inline(p, cmd_0, INLINE_POINTER_ASSIGN);
...
void SyntaxTree::AutoImplementAssign(Function *f, Class *t)
{
	...
	if (t->is_super_array){
		...
		Command *cmd_0 = add_command_const(nc);
		Command *cmd_assign0 = add_command_operator_by_inline(for_var, cmd_0, INLINE_INT_ASSIGN);
		f->block->commands.add(cmd_assign0);

		// while(for_var < self.num)
		Command *cmd_cmp = add_command_operator_by_inline(for_var, cp_command(other_num), INLINE_INT_SMALLER);
		...
		// ...for_var += 1
		Command *cmd_inc = add_command_operator_by_inline(for_var, cmd_0 /*dummy*/, INLINE_INT_INCREASE);
		...
		// while(for_var < self.num)
		Command *cmd_cmp = add_command_operator_by_inline(for_var, c_num, INLINE_INT_SMALLER);
...
// delete...
	...
	if (f_del){
		...
		constants[nc].setInt(0);
		Command *cmd_0 = add_command_const(nc);
		Command *cmd_assign = add_command_operator_by_inline(for_var, cmd_0, INLINE_INT_ASSIGN);
		f->block->commands.add(cmd_assign);

		// while(for_var < self.num)
		Command *cmd_cmp = add_command_operator_by_inline(for_var, self_num, INLINE_INT_SMALLER);
...
void SyntaxTree::AutoImplementArrayResize(Function *f, Class *t)
{
	...
	// num_old = self.num
	Command *cmd_copy_num = add_command_operator_by_inline(num_old, self_num, INLINE_INT_ASSIGN);
...
// delete...
	...
	if (f_del){
		// for_var = num
		Command *cmd_assign = add_command_operator_by_inline(for_var, num, INLINE_INT_ASSIGN);
		f->block->commands.add(cmd_assign);
		...
		// ...for_var += 1
		Command *cmd_inc = add_command_operator_by_inline(for_var, num /*dummy*/, INLINE_INT_INCREASE);
		b->commands.add(cmd_inc);
		f->block->commands.add(cb);
	}
	...
	if (f_init){
		// for_var = num_old
		Command *cmd_assign = add_command_operator_by_inline(for_var, num_old, INLINE_INT_ASSIGN);
		f->block->commands.add(cmd_assign);
...
void SyntaxTree::AutoImplementArrayAdd(Function *f, Class *t)
{
	...
	Command *cmd_add = add_command_operator_by_inline(self_num, cmd_1, INLINE_INT_ADD);
	...
	Command *cmd_sub = add_command_operator_by_inline(cp_command(self_num), cmd_1, INLINE_INT_SUBTRACT);
	Command *deref_self = deref_command(cp_command(self));
	Command *self_data = shift_command(deref_self, false, 0, t->parent->GetPointer());
